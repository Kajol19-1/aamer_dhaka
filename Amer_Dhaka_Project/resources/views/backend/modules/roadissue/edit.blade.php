...
<div class="row justify-content-center">
    <div class="col-md-8">
        <div class="card">
            ...
            <div class="card-body">
                <form action="{{ route('roadissue.edit.submit') }}" method="POST" enctype="multipart/form-data">
                    @csrf
                    {{-- keep the record id --}}
                    ...
                    <div class="mb-3">
                        ...
                        <select name="status" id="status" class="form-select @error('status') is-invalid @enderror">
                            <option value="Pending" {{ old('status', $roadissue->status) == 'Pending' ? 'selected' : '' }}>Pending</option>
                            <option value="In Process" {{ old('status', $roadissue->status) == 'In Process' ? 'selected' : '' }}>In Process</option>
                            <option value="Solved" {{ old('status', $roadissue->status) == 'Solved' ? 'selected' : '' }}>Solved</option>
                            <option value="Rejected" {{ old('status', $roadissue->status) == 'Rejected' ? 'selected' : '' }}>Rejected</option>
                        </select>
