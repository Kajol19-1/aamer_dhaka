...
@section('page_title', 'Illigal Structure Issue')
...
<div class="row justify-content-center">
    <div class="col-md-8">
        <div class="card">
            ...
            <div class="card-body">
                <form action="{{ route('illegalstructureissue.edit.submit') }}" method="POST" enctype="multipart/form-data">
                    @csrf
                    {{-- keep the record id --}}
                    ...
                    <div class="mb-3">
                        ...
                        <select name="status" id="status" class="form-select @error('status') is-invalid @enderror">
                            <option value="Pending" {{ old('status', $illegalstructureissue->status) == 'Pending' ? 'selected' : '' }}>Pending</option>
                            <option value="In Process" {{ old('status', $illegalstructureissue->status) == 'In Process' ? 'selected' : '' }}>In Process</option>
                            <option value="Solved" {{ old('status', $illegalstructureissue->status) == 'Solved' ? 'selected' : '' }}>Solved</option>
                            <option value="Rejected" {{ old('status', $illegalstructureissue->status) == 'Rejected' ? 'selected' : '' }}>Rejected</option>
                        </select>
                    ...
                    <div class="text-center">
                        ...
                        <a href="{{ route('illegalstructureissue.list') }}" class="btn btn-secondary ms-2">Cancel</a>
                    </div>
                </form>
            </div>
        </div>
